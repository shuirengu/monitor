(function($) {
        ...
        target.renderRow = function(tr,item){
            ...
            $.each(options.columns, function(index, column) {
                ...
                if(index==0&&column.field=='selectItem'){
                    ...
                    if(column.radio){
                        var _ipt = $('<input name="select_item" type="radio" value="'+item[options.menuId]+'"></input>');
                        td.append(_ipt);
                    }
                    if(column.checkbox){
                        var _ipt = $('<input name="select_item" type="checkbox" value="'+item[options.menuId]+'"></input>');
                        td.append(_ipt);
                    }
                    tr.append(td);
                }else{
    ...
    $.fn.bootstrapTreeTable.methods = {
        ...
        getSelections : function(target, data) {
            ...
            // 如果是radio
            if(_ipt.attr("type")=="radio"){
                chk_value.push({menuId:_ipt.val()});
            }else{
                _ipt.each(function(_i,_item){
                    chk_value.push({menuId:$(_item).val()});
                });
            }
            return chk_value;
        },
